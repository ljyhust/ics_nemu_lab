...
enum {
  _KEY_NONE = 0,
  MAP(_KEYS, _KEY_NAME) // _KEYS(_KEY_NAME) => _KEYS(_KEY_NAME) => _KEY_NAME(ESCAPE) _KEY_NAME(F1) ... => _KEY_ESCAPE  _KEY_F1
};

#define SDL_KEYMAP(k) [concat(SDL_SCANCODE_, k)] = concat(_KEY_, k),
static uint32_t keymap[256] = {
  MAP(_KEYS, SDL_KEYMAP) // => _KEYS(SDL_KEYMAP) => SDL_KEYMAP(ESCAPE) SDL_KEYMAP(F1) ... => SDL_SCANCODE_0 = _KEY_ESCAPE
};
...
void send_key(uint8_t scancode, bool is_keydown) {
  if (nemu_state.state == NEMU_RUNNING &&
    ...
    key_r = (key_r + 1) % KEY_QUEUE_LEN; // 取模运算 循环列表
