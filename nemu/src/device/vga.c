...
void vga_data_io_handler(uint32_t offset, int len, bool is_write) {
...
void init_vga() {
  ...
  add_pio_map("screen", VGACTL_PORT, (void *)vgactl_port_base, 8, vga_data_io_handler);
  add_mmio_map("screen", VGACTL_MMIO, (void *)vgactl_port_base, 8, vga_data_io_handler);
