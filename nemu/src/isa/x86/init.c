...
void init_isa() {
  ...
  /* why? statement function in this bulk means reg_test() function validate in this bulk
    if reg_test() not statement before. This is userable in C program: 先声明后使用*/
  void reg_test();
  reg_test();
  /* Load built-in image. */
